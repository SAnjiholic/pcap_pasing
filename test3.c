...
void packet_parsing(unsigned char *user, const struct pcap_pkthdr *h, const unsigned char *p);
...
int main(int argc, char *argv[]) {
	...
	for (dev = alldevs, j = 0; j< devnum-1; dev = dev->next,j++);
	...
	printf("\nListening on %s\n", dev->description);
	pcap_freealldevs(alldevs);
	pcap_loop(pcap, 0, packet_parsing, NULL);
	pcap_close(pcap);
	return 0;
}
...
void packet_parsing(unsigned char *user, const struct pcap_pkthdr *h, const unsigned char *p){
	...
	for(i=0;(data= pcap_next_ex(*user, &header, &p))!=NULL;i++){
//	for(i=0; p!=NULL; i++){
